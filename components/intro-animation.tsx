...
// The bolt is the mark's right-hand stroke — the same zigzag, traced as its
// own centerline so it can be drawn independently of the fill above.
const BOLT_PATH =
  "M 202.259 145.184 C 206.242 138.685, 221.400 113.947, 235.943 90.210 C 250.487 66.474, 262.508 47.175, 262.657 47.324 C 262.903 47.569, 256.102 67.311, 239.919 113.331 C 236.663 122.589, 234 130.351, 234 130.581 C 234 130.812, 245.454 131, 259.453 131 L 284.905 131 L 265.953 156 C 255.529 169.750, 238.471 192.250, 228.047 206 L 209.095 231";
...
export const IntroAnimation = () => {
  ...
  return (
    <motion.div
      ...
      initial={{ opacity: 1 }}
      role="presentation"
      transition={{ duration: OVERLAY_FADE / 1000, ease: [0.4, 0, 0.2, 1] }}
      onClick={skip}
    >
      <div className="relative h-28 w-auto sm:h-36">
        ...
        >
          ...
          />
          {/* Spark — draws the bolt segment of the M */}
          <motion.path
            animate={{ pathLength: boltDrawn ? 1 : 0 }}
            d={BOLT_PATH}
            ...
            style={{
              filter:
                "drop-shadow(0 0 8px #ED1C24) drop-shadow(0 0 18px #ED1C24)",
              opacity: ignited ? 0 : 1,
              transition: `opacity ${MARK_REVEAL}ms ease`,
            }}
            transition={{ duration: BOLT_DRAW / 1000, ease: [0.16, 1, 0.3, 1] }}
          />
        </svg>
        {/* Flash — the instant the spark ignites the full mark */}
        <motion.div
